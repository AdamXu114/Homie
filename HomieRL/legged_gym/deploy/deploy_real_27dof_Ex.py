...
sys.path.append('/workspaces/humanoid_g1')
...
import lcm
#from deploy.lcm_types.HeightMap_lcmt import HeightMap_lcmt
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        model = onnx.load("/home/xu/OpenHomie-main/HomieRL/legged_gym/deploy/pre_train/g1/policy.onnx")#policy_1_21dof_test.onnx"policy_1_21dof_normal_new5
        onnx.checker.check_model(model)
        self.policy = ort.InferenceSession("/home/xu/OpenHomie-main/HomieRL/legged_gym/deploy/pre_train/g1/policy.onnx")
        ...
        self.counter = 0
        self.hist_obs = deque(maxlen=config.num_obs_frame-1)
        ...
        for _ in range(10):
            action = self.policy.run(None, {'input': np.zeros([1, config.num_obs * config.num_obs_frame+9*13], dtype=np.float32)})[0]
        ...
        self.run_map_thread = threading.Thread(target=self.poll, daemon=False)
        self.run_map_thread.start()
        if config.msg_type == "hg":
            ...
            #self.height_map=self.lc.subscribe("/pim/height_map", self.HeightmapHandler)
    ...
    # def HeightmapHandler(self, channel, data):
    #     msg = HeightMap_lcmt.decode(data)
    #     self.height_map=np.array(msg.data,dtype=np.float32)
    #
    #    # print(self.height_map-0.5,"dddddddddddd")
    ...
    def run(self, dry_run=False):
      ...
      #  print(self.remote_controller.ly)
        self.cmd[0] =self.remote_controller.ly#min(self.remote_controller.ly,0.5)
        ...
        # v_mask=np.linalg.norm(self.cmd)<0.05
        ...
        # TODO: change the observation，按照mujoco仿真的顺序写
        # self.obs[:3]=ang_vel
        ...
        self.obs[3] = 0.76  # height
        self.obs[4:7] = ang_vel
        self.obs[7:10] = gravity_orientation
        self.obs[10: 10 + num_dofs] = qj_obs
        self.obs[10 + num_dofs: 10 + 2 * num_dofs] = dqj_obs
        self.obs[10 + 2 * num_dofs: 10 + 2 * num_dofs + num_actions] = self.action
        
        # self.obs[2:5]= self.cmd * self.config.cmd_scale * self.config.max_cmd
        # self.obs[5:5 + num_actions]=qj_obsb.yaml
        ...
        obs_copy= np.clip(self.obs, -50, 50)
        ...
        # TODO: 注释掉了heightmap部分
        # terrain=np.ones(13*9,dtype=np.float32)*0.3
        # terrain=self.height_map-0.5#0.73-self.height_map-0.5
        # terrain[terrain<-0.3]=0.28
        # terrain=np.clip(terrain, -0.3,1.)
       # print(self.height_map-0.5,"heiiiig")
        

        # hist_obs.popleft()
        # TODO: change the policy input
        # policy_input = np.concatenate([obs_hist, obs_copy,terrain])[None, :]
        policy_input = obs_hist.astype(np.float32)[None, :]
        # policy_input = torch.from_numpy(policy_input).float().unsqueeze(0)
        # obs_tensor = torch.from_numpy(obs).unsqueeze(0)
        # policy inference
    
        self.action = self.policy.run(None, {'input': policy_input})[0].squeeze()
        #print(self.action,"aac")
        ...
        # TODO: change the action,上下肢分开的，不需要改动，在yaml文件里需要分开写
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if controller.remote_controller.button[KeyMap.select] == 1 or controller.remote_controller.button[KeyMap.F2] == 1 :
                break
