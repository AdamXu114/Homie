...
from deploy.lcm_types.BotStatus_lcmt import BotStatus_lcmt
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        model = onnx.load("/workspaces/humanoid_g1/deploy/pre_train/g1/policy_lab_poscmd11.onnx")#policy_1_21dof_test.onnx"policy_1_21dof_normal_new5
        onnx.checker.check_model(model)
        self.policy = ort.InferenceSession("/workspaces/humanoid_g1/deploy/pre_train/g1/policy_lab_poscmd11.onnx")
        ...
        if config.msg_type == "hg":
            ...
            self.poscmd=self.lc.subscribe("/pim/bot_status", self.PoscmdHandler)
    ...
    def HeightmapHandler(self, channel, data):
        ...
        #print(msg.data,"dddddddddddd")
    
    def PoscmdHandler(self, channel, data):
        msg = BotStatus_lcmt.decode(data)
        self.poscmd=np.array(msg.goal,dtype=np.float32)[:2]
    ...
    def run(self, dry_run=False):
       ...
       # t0=time.time()
        ...
        self.obs[6:8]=self.poscmd
        self.obs[8]=np.clip(self.time_left,a_min=0.,a_max=1.0)
        ...
        terrain=np.ones(13*9,dtype=np.float32)*0.23
        #terrain=0.73-self.height_map-0.5
        terrain=np.clip(terrain, -1.,1.)
       # print(self.height_map,"heiiiig")
