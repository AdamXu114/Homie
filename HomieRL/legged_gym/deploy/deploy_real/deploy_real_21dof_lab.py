...
from unitree_sdk2py.idl.unitree_go.msg.dds_ import HeightMap_ as HeightMap
...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        model = onnx.load("/workspaces/humanoid_g1/deploy/pre_train/g1/policy_labnew4.onnx")#policy_1_21dof_test.onnx"policy_1_21dof_normal_new5
        onnx.checker.check_model(model)
        self.policy = ort.InferenceSession("/workspaces/humanoid_g1/deploy/pre_train/g1/policy_labnew4.onnx")
        ...
        for _ in range(10):
            action = self.policy.run(None, {'obs': np.zeros([1, config.num_obs * config.num_obs_frame], dtype=np.float32)})[0]
        ...
        if config.msg_type == "hg":
            ...
            # heightmap_topic="/pim/cloud_aligned"
            # self.heightmap_subscriber= ChannelPublisher(heightmap_topic,HeightMap)
            # self.heightmap_subscriber.Init(self.HeightmapHandler, 10)
    ...
    def HeightmapHandler(self, msg: HeightMap):
        self.heigtmap= msg.data
        print(self.heigtmap)
        #self.remote_controller.set(self.low_state.wireless_remote)
    ...
    def run(self, dry_run=False):
        ...
        self.action = self.policy.run(None, {'obs': policy_input})[0].squeeze()
