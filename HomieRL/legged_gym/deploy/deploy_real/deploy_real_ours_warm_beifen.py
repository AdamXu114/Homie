...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        for _ in range(10):
            action =self.policy(torch.zeros(1,470)).detach().numpy().squeeze()
    ...
    def run(self, dry_run=False):
        ...
        policy_input= torch.from_numpy(policy_input).unsqueeze(0).float()
        #hist_obs.append(obs_copy)
        # hist_obs.popleft()
        self.action = self.policy(policy_input).detach().numpy().squeeze()
        
    #     self.obs = np.clip(self.obs, -18, 18)
    #    # obs = np.clip(self.obs, -18, 18)
