...
class ArmActorCritic(nn.Module):
    ...
    def __init__(self, num_obs,
                 num_privileged_obs,    # 暂定
                 num_obs_history,   # upper_dof，action, rpy，height, upper_commands
                 num_actions,   # upper_dof，rpy，height
                 **kwargs):
        if kwargs:
    ...
    def evaluate(self, observation_history, privileged_observations, **kwargs):
        ...
        #print(f"obs_h shape: {obs_h.shape}, obs shape: {obs.shape}, privileged_observations shape: {privileged_observations.shape}")
